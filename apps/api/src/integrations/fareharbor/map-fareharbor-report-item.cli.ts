...
async function main(): Promise<void> {
  ...
  if (!command) {
    console.error(
      'Usage: npm run map:fareharbor-report-item -- --item-label "Goat Recess" --name "Experience name"',
    );
    console.error(
      '   or: npm run map:fareharbor-report-item -- --item-label "Goat Recess" --experience-id <uuid>',
    );
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    const result = await mapper.mapReportItem(command);
    printResult(result);
...
function parseArgs(
  ...
  | { itemLabel: string; name: string }
  | { itemLabel: string; experienceId: string }
  | undefined {
  ...
  if (trimmedName && !trimmedExperience) {
    return { itemLabel: trimmedLabel, name: trimmedName };
  }
  if (trimmedExperience && !trimmedName) {
    return { itemLabel: trimmedLabel, experienceId: trimmedExperience };
  }
  return undefined;
}

function printResult(
  result: Awaited<ReturnType<FareharborExperienceMapService["mapReportItem"]>>,
): void {
  if (result.outcome === "created") {
    if (result.createdExperience) {
      console.log(
        `Created experience ${result.experienceId} and mapped FareHarbor report item ${result.itemId}`,
      );
      return;
    }
    console.log(
      `Mapped FareHarbor report item ${result.itemId} to experience ${result.experienceId}`,
    );
    return;
  }
  ...
  if (result.outcome === "unchanged") {
    console.log(
      `FareHarbor report item ${result.itemId} already maps to experience ${result.experienceId}`,
    );
    return;
  }
  ...
  if (result.outcome === "conflict") {
    console.error(
      `Refused: FareHarbor report item ${result.itemId} is already mapped to experience ${result.mappedExperienceId}. Explicit remap is not implemented.`,
    );
    return;
  }
