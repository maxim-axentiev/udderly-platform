...
Later mapping: FH `receipt_subtotal` / `receipt_taxes` / `receipt_total`. Square **top-level** order totals excluding tip for original sale economics; Square refunds are separate (`docs/square.md`). Do not use `net_amounts` as canonical sale totals. A Square return-only or return-adjustment order is snapshot evidence, not a `sale`.
